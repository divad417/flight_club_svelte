...
export const onBeerUpdate = functions.firestore
  .document('beers/{beerId}')
  .onWrite(async (change, context) => {
    const session = change.after.exists
      ? change.after.get('session')
      : change.before.get('session');
...
export const updateAll = functions.firestore
  .document('testing/{id}')
  .onWrite(async (change, context) => {
...
async function updateUser(user: string) {
  ...
  const userData = {
    ...
    avg_score:
      beers.map((beer) => beer.score).reduce((curr, next) => curr + next, 0) / beers.length,
    avg_abv: beers.map((beer) => beer.abv).reduce((curr, next) => curr + next, 0) / beers.length,
    count: beers.length,
